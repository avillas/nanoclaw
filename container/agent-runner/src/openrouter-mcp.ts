 ...
 *
 * Launched as a child process by the agent-runner and registered with the SDK.
 * Credentials arrive via OPENROUTER_API_KEY env var (injected by the host).
 *
...
interface OpenRouterResponse {
  ...
  model?: string;
  // OpenRouter includes cost data in some responses
  total_cost?: number;
}
...
async function openrouterChat(
  ...
  // Write cost record for the host to pick up
  writeCostRecord({
...
Popular models: anthropic/claude-sonnet-4, openai/gpt-4o, google/gemini-2.5-pro, deepseek/deepseek-chat-v3, meta-llama/llama-4-maverick, mistralai/mistral-large`,
  {
    prompt: z
      .string()
      .describe('The user/task prompt to send to the model'),
    model: z
      .string()
      .optional()
      .describe(
        `OpenRouter model ID (e.g. "openai/gpt-4o", "google/gemini-2.5-pro"). Defaults to ${OPENROUTER_DEFAULT_MODEL}`,
      ),
    system: z
      .string()
      .optional()
...
server.tool(
  ...
  `Send a multi-turn conversation to any OpenRouter model. Use when you need to provide conversation history (e.g. few-shot examples, prior context).`,
  {
    ...
    model: z
      ...
      .optional()
      .describe(
        `OpenRouter model ID. Defaults to ${OPENROUTER_DEFAULT_MODEL}`,
      ),
...
main().catch((err) => {
  process.stderr.write(`OpenRouter MCP server fatal: ${err}\n`);
  process.exit(1);
});
